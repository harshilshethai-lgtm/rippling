...
/**
 * Map step for the Select People mode.
 * Shows identity-only column mapping (ProfileNumber / Name / Email / Ignore).
 *
 * mapping: { ProfileNumber?: colIdx, Name?: colIdx, Email?: colIdx }
 */
export default function MapStepIdentity({ headers, mapping, onMappingChange }) {
  ...
  function assignedField(colIndex) {
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[12.5px] text-rippling-muted mt-0.5 leading-relaxed">
          Review the mapping from your CSV and ensure they are the right fields. Map at least one
          identity column (profile number, name, or email).
        </p>
      </div>
      ...
      {!isValid && (
        <p className="text-[12px] text-amber-700 bg-amber-50 border border-amber-200 rounded px-3 py-2">
          Map at least one column to <strong>Rippling profile number</strong>, <strong>Name</strong>,
          {' '}or <strong>Email</strong> to continue.
        </p>
      )}
    </div>
  )
